...
#    This sets up the server listener
    # thread = threading.Thread(target=app.run(hostip, port), kwargs={'hostip':hostip, 'port':port, 'debug':true})

    thread = threading.Thread(target=app.run(hostip, port, debug=True))
    thread.start()

    print "app is running"
    
    t2 = threading.Thread(target=client_loop(hostip, port))
    t2.start()

#    client_loop(hostip, port)